...
def ant_colony_optimization(customers, vehicles, depot, n_ants, n_iterations, alpha, beta, evaporation_rate, Q):
	...
	pheromone = np.ones((n_customers, n_customers))
	...
	for iteration in range(n_iterations):
		...
		for ant in range(n_ants):
			...
			current_customer = np.random.randint(n_customers)
			visited[current_customer] = True
			route.visit(customers[current_customer])
			...
			while False in visited:
				...
				if np.sum(probabilities) > 0:
					...
					route.visit(customers[next_customer])
...
if __name__ == "__main__":
	...
	depot = Coordinate(-1, 4.4184, 114.0932)
	customers = [
		Customer(1, 4.3555, 113.9777, 5),
		Customer(2, 4.3976, 114.0049, 8),
		Customer(3, 4.3163, 114.0764, 3),
		Customer(4, 4.3184, 113.9932, 6),
		Customer(5, 4.4024, 113.9896, 5),
		Customer(6, 4.4142, 114.0127, 8),
		Customer(7, 4.4804, 114.0734, 3),
		Customer(8, 4.3818, 114.2034, 6),
		Customer(9, 4.4935, 114.1828, 5),
		Customer(10, 4.4932, 114.1322, 8)
	]
	vehicles = [
		Vehicle('A', 25, 1.2),
		Vehicle('B', 30, 1.5)
	]
	path, length = ant_colony_optimization(customers, vehicles, depot, n_ants=50, n_iterations=500, alpha=1, beta=1, evaporation_rate=0.5, Q=1)
	...
	print("%-*s : %-*s" % (15, "Total Distance", 15, total_distance))
	print("%-*s : %-*s" % (15, "Total Cost", 15, total_cost))
	print("\n")
	print(detail)
